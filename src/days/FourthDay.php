...
class FourthDay
{
     ...
     * @return int
     */
    public function first(string $input): int
    {
        $passwords = $this->getPasswordList($input);
        
        $validCnt = 0;
        
        foreach ($passwords as $password) {
            $unique = [];
            $passParts = explode(' ', trim($password));
            
            foreach ($passParts as $passPart) {
                $unique[$passPart] = null;
            }
            
            if (count($passParts) === count($unique)) {
                ++$validCnt;
            }
        }
     ...
     * @return int
     */
    public function second(string $input): int
    {
        $passwords = $this->getPasswordList($input);
        ...
        foreach ($passwords as $password) {
            ...
            foreach ($passParts as $passPart) {
                $passPartArray = str_split($passPart);
                sort($passPartArray);
                $passPart = implode('', $passPartArray);
     ...
     * @return array
     */
    protected function getPasswordList(string $passwords): array
    {
        return explode("\n", $passwords);
    }
}
